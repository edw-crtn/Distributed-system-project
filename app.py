...
# Build mongo URI for replica set (no auth for dev)
...
def vote():
    ...
    # get current elos
    winner = characters.find_one({"_id": mongo_client.get_default_database().codec_options.document_class.__mro__[0]})  # placeholder avoid linter
    ...
    # Update DB atomically (two updates)
    characters.update_one({"_id": ObjectId(winner_id)}, {"$set": {"elo": new_w}})
    characters.update_one({"_id": ObjectId(loser_id)}, {"$set": {"elo": new_l}})

    # Invalidate cache so next reads show fresh elos
    invalidate_cache()

    # return result page showing elos + delta
    return render_template("result.html",
                           winner= {"_id": winner_id, "name": winner["name"], "image": winner.get("image"), "old_elo": w_elo, "new_elo": new_w},
                           loser= {"_id": loser_id, "name": loser["name"], "image": loser.get("image"), "old_elo": l_elo, "new_elo": new_l},
                           delta=delta)

@app.route("/health")
def health():
    # simple health: db + redis reachable
    ok = True
    msgs = []
    try:
